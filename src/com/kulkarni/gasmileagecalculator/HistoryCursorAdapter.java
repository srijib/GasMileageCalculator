...
public class HistoryCursorAdapter extends CursorAdapter {
	...
	public HistoryCursorAdapter(Context context, Cursor c, int flags) {
		...
		app          = (RefuelerApplication) context.getApplicationContext();
	...
	public void bindView(View view, Context context, Cursor cursor) {
		...
		String vol_txt = String.format(Locale.getDefault(),"%.2f %s", volume, resources.getString(R.string.volume_unit));
		txtVolume.setText(vol_txt);
		
		double rate = cursor.getDouble(cRateIndex);
		String rate_txt = String.format(Locale.getDefault(), "%s%.2f per %s", resources.getString(R.string.currency_symbol), rate,
																			  resources.getString(R.string.volume_unit));
		txtFuelRate.setText(rate_txt);
		
		double cost = cursor.getDouble(cCostIndex);
		String cost_txt = String.format(Locale.getDefault(), "%s%.2f", resources.getString(R.string.currency_symbol), cost);
	}
